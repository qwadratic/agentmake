...
Run artifacts in this directory are committed as they landed (`goal.md`,
`Makefile`, `src/`). This is the blow-by-blow the README summarizes.

## The run

The board's task-14 was pulled by `make board-next`, decomposed by the engine
into 7 components — stub agent, plan gate, components generator, engine core
...
on the board, and rerunning resumed at the gate that failed — the jq gate
rejected it twice; retries are paid. The fix and the follow-up hardening task
(task-15) are on the board.
