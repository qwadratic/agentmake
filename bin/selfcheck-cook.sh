#!/usr/bin/env bash
# selfcheck: bin/cook — mock runtime ONLY, zero LLM calls.
...
"$COOK" --runtime mock "game of life, make it look alive" > run.txt 2>&1 \
...
grep -q "cooked -> ./$d" run.txt                         || fail "no artifact census"
...
"$COOK" --dry --runtime mock --dir dry-run "a pomodoro timer, keyboard only" > dry.txt 2>&1 \
...
evil='$(rm -rf /tmp/cook-pwned) ../../etc/passwd `x` "q"'
before=$(ls -d */ | sort)
"$COOK" --dry --runtime mock "$evil" > evil.txt 2>&1 \
...
[ ! -e /tmp/cook-pwned ]              || fail "injection EXECUTED"
grep -q 'rm -rf' "$new/Makefile" && fail "goal text leaked into Makefile"
"$COOK" --dry --runtime mock '!!!' >/dev/null 2>&1 && fail "empty slug accepted"
...
"$COOK" --resume dry-run --runtime mock > resume.txt 2>&1 \
...
"$COOK" --dry --runtime mock --tier prd --dir tiered "tiny thing" >/dev/null 2>&1 \
...
if command -v backlog >/dev/null; then
  ...
  "$COOK" --board "selfcheck board task — cook one-shot round-trip" > board.txt 2>&1 \
...
"$COOK" progress "$d" > prog.txt || fail "progress passthrough exited nonzero"   # capture, don't pipe: grep -q SIGPIPEs make
grep -q '100%' prog.txt || fail "progress passthrough: no 100% bar"
echo "PASS 7 progress passthrough"

echo "PASS: all cook selfchecks"
